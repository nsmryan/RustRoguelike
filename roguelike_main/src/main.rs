...
pub fn take_screenshot(game: &mut Game, display: &mut Display) -> Result<(), String> {
    ...
    game.step_game(InputAction::None);
    render_all(&mut display.panels, &mut display.state, &display.sprites, game, 0.1)?;
...
fn update_display(game: &mut Game, display: &mut Display, dt: f32) -> Result<(), String> {
    ...
    /* Draw the Game to the Screen */
    {
        ...
        render_all(&mut display.panels, &mut display.state, &display.sprites, game, dt)?;
...

    {
        ...
        display.draw_all(game.data.map.size(), &mut game.rng, &game.config, &game.settings);
